...
<div class="container mx-auto">
    <h1 class="text-2xl font-bold mb-4">{{ __('Create API Key') }}</h1>

    <form action="{{ route('user.api-keys.store') }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        @csrf
        <div class="mb-4">
        ...
        <div class="flex items-center justify-between">
            <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" type="submit">{{ __('Create API Key') }}</button>
        </div>
    </form>
</div>
@endsection
